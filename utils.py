...
import numpy as np
import cv2
import loadData2_vgg_crossout as loadData
...
def writePredict(folder_name, epoch, index, pred, flag): # [batch_size, vocab_size] * max_output_len
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
...
def writeLoss(folder_name, loss_value, flag):
    if not os.path.exists(folder_name):
        os.makedirs(folder_name)
    if flag == 'train':
